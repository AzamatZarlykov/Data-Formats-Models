...
CREATE
(NickOrange)-[:STUDIES]->(Philosophy),
(NickOrange)-[:IS_PART_OF]->(Kcl),
(JimPurple)-[:STUDIES]->(ComputerScience),
(JimPurple)-[:IS_PART_OF]->(Mit),
(MarkBlue)-[:STUDIES]->(Economics),
(MarkBlue)-[:IS_PART_OF]->(Cuni),
(AlexGreen)-[:IS_PART_OF]->(Cuni),
(AlexGreen)-[:PARTICIPATES_IN]->(ResearchAboutDeterministicAlgorithms),
(AlexGreen)-[:PARTICIPATES_IN]->(ResearchAboutConvexHulls),
(AlexGreen)-[:IS_AUTHER_OF]->(PaperAboutMathematicalAnalysis),
(SamRed)-[:IS_PART_OF]->(Kcl),
(SamRed)-[:IS_AUTHER_OF]->(PaperAboutLinearAlgebra),
(SamRed)-[:IS_AUTHER_OF]->(PaperAboutDiscreteMathematics),
(SamRed)-[:PARTICIPATES_IN]->(ResearchAboutRandomizedAlgorithms),
